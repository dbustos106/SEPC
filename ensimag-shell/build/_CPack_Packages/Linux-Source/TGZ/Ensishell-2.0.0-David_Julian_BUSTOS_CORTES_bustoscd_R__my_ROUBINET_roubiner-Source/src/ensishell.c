...
/*
	Fonction qui configure les gestionnaires de signaux
*/
void setup_signal_handler(){
	/* Configurer le gestionnaire du signal SIGCHLD */
    ...
    /* Configurer le gestionnaire du signal SIGXCPU */
...
void sigchld_handler(){
    ...
    while ((child_pid = waitpid(-1, &status, WNOHANG)) > 0) {  
		...
		/* Trouver le processus dans la liste des jobs */
	...
	for(int i = 0; i < seq_len; i++){
		...
		if(pid == 0){	
			...
			if(limit_initialized){
				/* Limitez le temps d'utilisation du processeur */
				if (setrlimit(RLIMIT_CPU, &limit) != 0) error_message("error en setrlimit");
			}

			/* Changer le programme exécuté par le fil */
			execvp(cmd[0], cmd);
...
char* etendre_line(char* line){
	...
	do{
		/* Obtenir le pointeur vers le mot suivant */
		...
		if(expanded_cmd){
			/* Concaténer le mot étendu à la commande étendue */
...
int open_input_fd(struct cmdline *l){
	...
	if(l->in != NULL){
	    ...
	    if(input_fd == -1) error_message("erreur en open");
	}
	return input_fd;
}
...
int open_output_fd(struct cmdline *l){
	...
	if(l->out != NULL){
	    ...
	    if(output_fd == -1) error_message("erreur en open");
		
		/* Tronquer le fichier à une longueur de 0 pour effacer le contenu existant */
    	if(ftruncate(output_fd, 0) == -1) error_message("erreur en ftruncate");
	}
	return output_fd;
}
...
void afficher_jobs(){
	...
	while(current_job != NULL){
		/* Si le processus actuel continue en arrière-plan */
		if(waitpid(current_job->pid, &status, WNOHANG) == 0){
			printf("PID: %d\t Command: ", current_job->pid);

			/* Imprimer la liste des commandes du processus actuel */			
...
void ajouter_job(pid_t pid, char** cmd){
	...
	/* Si un nœud existe à la tête, 
	définir son précédent comme le nouveau nœud */
	if(jobs != NULL){
		jobs->prev = new_job;
	}

	/* Ajouter le nœud au début de la liste */
	new_job->next = jobs;
	new_job->prev = NULL;
	jobs = new_job;
...
void supprimer_job(struct Job *job){
	...
	/* Si le nœud est le début de la liste */
	if(job->prev == NULL){
		jobs = job->next;
	}else{
	...
	/* Si le nœud n'est pas la fin de la liste */
	if(job->next != NULL){
